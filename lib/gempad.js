...
llamado(cb){
        this.instance.post(GEM.pllamado,this.inicialflt,{headers:{Cookie:this.cookie}}).then(s=>{
        if(this.debug) console.log(s)
            cb(s.data.data)
    }).catch(err=>{
        console.log("ocurrió un error en llamado...intentando nuevamente.")
        setTimeout(function(){gem.llamado(x=>cb(x))},2000)
})
}

llamadod(cb){
        this.instance.post(GEM.pllamadod,this.inicialflt,{headers:{Cookie:this.cookie}}).then(s=>{  
           if(this.debug)console.log(s)
            cb(s.data.data)
    }).catch(err=>{console.log("ocurrió un error en llamadod")
})
}
